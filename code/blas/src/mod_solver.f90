module mod_solver
  use mod_param, only: wp, &
       one, &
       zero
  use mod_matops, only: mydgemm,&
       LDUdecompose, &
       prettyprint
  private

  public :: jacobi_method
...
contains
  ...
  subroutine jacobi_method(silent,itermax,n,A,b,xbegin,xtol,ans,error,niters)
    implicit none
    ...
    call mydgemm(n,n,n,one,one,-Dinverse,L+U,Q)
    call mydgemm(n,1,n,one,one,Dinverse,b,C)
    xk = xbegin
    ...
    if (silent) then 
       do i = 1, itermax
          xkp1 = C
          call mydgemm(n,1,n,one,one,Q,xk,xkp1)
          xnorm = zero
          do j = 1, n
